#!/usr/bin/env python3
"""
NaviCare Lab Availability Updater
Simple script to update only the availability information for existing lab facilities
"""
...
from lab_crawler import LabCrawler, LabCrawlConfig, LabTransformer
...
async def update_lab_availability(crawler: LabCrawler, lab_record: dict):
    """Update availability information for a single lab"""
    try:
        # Defensive: lab_record should be a dict
        if not isinstance(lab_record, dict):
            logger.error(f"Unexpected lab_record type: {type(lab_record)}. Payload: {repr(lab_record)[:500]}")
            return False
        ...
        if not existing_facility:
            logger.info(f"Lab facility not found in database: {facility_name} ({facility_slug}). Creating full record via crawler.")
            try:
                # Use the crawler's full facility processing to create facility and related records
                await crawler.process_lab(lab_record)
                logger.info(f"Created lab facility from Cortico record: {facility_name} ({facility_slug})")
                return True
            except Exception as e:
                logger.error(f"Failed to create lab facility from Cortico record {facility_name}: {e}")
                return False
        
        facility_id = existing_facility['id']
        
        # For labs, we don't have specific availability data like clinics
        # We'll just update the last checked timestamp
        logger.info(f"Updated last checked timestamp for lab: {facility_name}")
        return True
            
    except Exception as e:
        logger.error(f"Error updating lab {facility_name}: {e}")
        return False

async def fetch_and_update_lab_availability(config: LabCrawlConfig):
    """Fetch and update availability for all lab facilities"""
    logger.info("🚀 Starting NaviCare Lab Availability Update")
    logger.info("=" * 50)
            ...
            for i in range(0, len(results), config.batch_size):
                ...
                async def process_with_semaphore(record):
                    async with semaphore:
                        success = await update_lab_availability(crawler, record)
    ...
    logger.info("=" * 60)
    logger.info("FINAL LAB AVAILABILITY UPDATE STATISTICS")
    logger.info("=" * 60)
...
async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='NaviCare Lab Availability Updater')
    ...
    try:
        ...
        stats = await fetch_and_update_lab_availability(config)
        
        if stats['errors'] > 0:
            logger.warning(f"Completed with {stats['errors']} errors")
            # sys.exit(1)
        else:
            logger.info("✅ Lab availability update completed successfully!")

        sys.exit(0)
        
    except KeyboardInterrupt:
        logger.info("⏹️  Lab availability update interrupted by user")
    except Exception as e:
